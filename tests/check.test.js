...
describe('Helper functions', () => {
    ...
    describe('cleanupUndefinedList()', () => {
        it('Correct return values', () => {
            assert.deepEqual(
                updateProgress.cleanupUndefinedList(['Example1']),
                ['Example1']
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList(['Example2', undefined, undefined]),
                ['Example2']
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList([undefined, 'Example3', undefined]),
                ['Example3']
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList([undefined, undefined]),
                undefined
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList([]),
                undefined
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList(['Example1', undefined, 'Example2']),
                ['Example1', 'Example2']
            )
            assert.deepEqual(
                updateProgress.cleanupUndefinedList(['Example1', undefined, 'Example2', undefined], a => a.join(',')),
                'Example1,Example2'
            )
        })
    })
...
describe('CLI functions', () => {
    describe('parseCliArgs()', () => {
        ...
        it('Correct parsing of process exiting CLI arg --version', () => {
            ...
            process.exit.restore()
            assert(console.log.called)
            assert(console.log.calledWith(`${updateProgress.versionNumberProgramMajor}.${updateProgress.versionNumberProgramMinor}.${updateProgress.versionNumberConfig}`))
            console.log.restore()
        })
...
describe('Course Progress Render functions', () => {
    ...
    describe('getAllAchievedPoints()', () => {
        it('Correct counting of achieved submission points', () => {
            ...
            assert.equal(updateProgress.getAllValidSubmittedSubmissions(exercisesExampleLists02, requirementsExampleMinimumPointsPerSubmission02), 1)
            assert.equal(updateProgress.getAllValidSubmittedSubmissions(exercisesExampleLists03, requirementsExampleMinimumPointsPerSubmission02), 1)
